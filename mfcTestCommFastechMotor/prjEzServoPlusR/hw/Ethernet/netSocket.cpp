...
netSocket::netSocket(netSocket::cfg_t& cfg) :m_SocketCnt(0), m_clientSocket(0), m_clientEvent()
...
netSocket::err_e netSocket::Open()
{
  netSocket::err_e ret = netSocket::err_e::SUCCESS;
  if (m_IsOpened)
    return ret;
...
int netSocket::SendData(const char* p_str, uint32_t length)
{
  int result = 0;
  SOCKET sock;
